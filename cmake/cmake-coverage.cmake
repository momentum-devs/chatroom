...
# ~~~
#
# Example 3: Target added to the 'ccov' and 'chatroom-ccov-all' targets
#
# ~~~
# add_code_coverage_all_targets(EXCLUDE test/*) # Adds the 'chatroom-ccov-all' target set and sets it to exclude all files in test/ folders.
#
# add_executable(theExe main.cpp non_covered.cpp)
# target_code_coverage(theExe AUTO ALL EXCLUDE non_covered.cpp test/*) # As an executable target, adds to the 'ccov' and chatroom-ccov-all' targets, and the reports will exclude the non-covered.cpp file, and any files in a test/ folder.
# ~~~
...
# chatroom-ccov-all : Generates HTML code coverage report, merging every target added with 'ALL' parameter into a single detailed report.
#
# LLVM-COV:
# ccov : Generates HTML code coverage report for every target added with 'AUTO' parameter.
...
# chatroom-ccov-all : Generates HTML code coverage report, merging every target added with 'ALL' parameter into a single detailed report.
# chatroom-ccov-all-report : Prints summary per-file coverage information for every target added with ALL' parameter to the command line.
# chatroom-ccov-all-export : Exports the coverage report to a JSON file.
#
# Required:
...
# ALL - Adds the target to the 'chatroom-ccov-all' and 'chatroom-ccov-all-report' targets, which merge several executable targets coverage data to a single report. Effective on executable targets.
    ...
    if (CODE_COVERAGE)
        ...
        if (target_type STREQUAL "EXECUTABLE")
            ...
            if (target_code_coverage_ALL)
                if (NOT TARGET chatroom-ccov-all-processing)
                    message(
                            FATAL_ERROR
                            "Calling target_code_coverage with 'ALL' must be after a call to 'add_code_coverage_all_targets'."
                    )
                ...
                add_dependencies(chatroom-ccov-all-processing
                        ccov-run-${target_code_coverage_COVERAGE_TARGET_NAME})
            endif ()
        endif ()
    endif ()
...
# Adds the 'chatroom-ccov-all' type targets that calls all targets added via
...
# reports. Also adds the chatroom-ccov-all-capture Generates an all-merged.info file, for
...
function(add_code_coverage_all_targets)
    ...
    if (CODE_COVERAGE)
        if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang"
            ...
            if (WIN32)
                add_custom_target(
                        ...
                        ${EXCLUDE_REGEX}
                        DEPENDS chatroom-ccov-all-processing)
            else ()
                add_custom_target(
                        chatroom-ccov-all-report
                        COMMAND
        ...
        elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES
            ...
            add_custom_target(chatroom-ccov-all-processing COMMAND ;)
            ...
            add_custom_target(
                    ...
                    DEPENDS chatroom-ccov-all-capture)

        endif ()

        add_custom_command(
                TARGET chatroom-ccov-all
                POST_BUILD
                COMMAND ;
                COMMENT
