...
TEST_F(CreateFriendshipCommandImplIntegrationTest, createFriendship)
{
...

    {
        ...
        std::shared_ptr<Friendship> foundFriendship(db->query_one<Friendship>(
            FriendshipQuery::user->id == user->getId() && FriendshipQuery::user_friend->id == userFriend->getId()));
...
TEST_F(CreateFriendshipCommandImplIntegrationTest, throwsAnError_whenFriendshipAlreadyExists)
{
    ...
    ASSERT_THROW(createFriendshipCommandHandler.execute({userId, userFriendId}), errors::OperationNotValidError);
}
